...
def get_dtype_fmt_options(__inputs__, __outputs__):
    options = []
    input_names = ['xyz', 'center_xyz', 'xyz_batch_cnt', 'center_xyz_batch_cnt']
    output_names = ['idx']
...
def _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, min_radius, max_radius, sample_num):
    __inputs__ = []
    for arg in [xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__]:
        if arg != None:
    ...
    if min_radius != None:
        ...
        attr["dtype"] = "float"
        attr["value"] = min_radius
        __attrs__.append(attr)
    if max_radius != None:
        attr = {}
        attr["name"] = "max_radius"
        attr["dtype"] = "float"
        attr["value"] = max_radius
        __attrs__.append(attr)
    if sample_num != None:
        attr = {}
        attr["name"] = "sample_num"
        attr["dtype"] = "int"
        attr["value"] = sample_num
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("BallQuery", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def ball_query(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__=None, center_xyz_batch_cnt_in__=None, idx_out_=None, min_radius=0.0, max_radius=0.0, sample_num=0, kernel_name="ball_query", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, min_radius, max_radius, sample_num)
    ...
    origin_func_name = "ball_query"
    ascendc_src_dir_ex = "ball_query"
    ascendc_src_dir = "ball_query"
    ascendc_src_file = "ball_query.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BallQuery. kernel name is " + kernel_name)
    op_type = "BallQuery"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__], origin_outputs = [idx_out_], param_type_dynamic = False,\
        mc2_ctx = None, param_type_list = ['required', 'required', 'optional', 'optional', 'required'])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__=None, center_xyz_batch_cnt_in__=None, idx_out_=None, min_radius=0.0, max_radius=0.0, sample_num=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, min_radius, max_radius, sample_num)
    result = check_op_cap("op_select_format", "BallQuery", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__=None, center_xyz_batch_cnt_in__=None, idx_out_=None, min_radius=0.0, max_radius=0.0, sample_num=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, xyz_batch_cnt_in__, center_xyz_batch_cnt_in__, idx_out_, min_radius, max_radius, sample_num)
    result = check_op_cap("get_op_specific_info", "BallQuery", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
