...
[[maybe_unused]] uint32_t NNOPBASE_BallQuery = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnBallQueryTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "BallQuery";
    char inputDesc[] = {1, 1, 0, 0};
    char outputDesc[] = {1};
    char attrDesc[] = {1, 1, 1};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(xyz);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(centerXyz);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, xyz, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, centerXyz, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, xyzBatchCntOptional, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, centerXyzBatchCntOptional, 3));
    float tmp0 = static_cast<float>(minRadius);
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&tmp0), sizeof(float), 0, kNnopbaseFloat));
    float tmp1 = static_cast<float>(maxRadius);
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&tmp1), sizeof(float), 1, kNnopbaseFloat));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&sampleNum), sizeof(int64_t), 2, kNnopbaseInt));
