...
#include "aclnn_depth_to_space.h"
...
[[maybe_unused]] uint32_t NNOPBASE_DepthToSpace = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnDepthToSpaceTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "DepthToSpace";
    char inputDesc[] = {1};
    char outputDesc[] = {1};
    char attrDesc[] = {1, 0, 0};
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&blockSize), sizeof(int64_t), 0, kNnopbaseInt));
    if (modeOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(modeOptional), strlen(modeOptional) + 1, 1, kNnopbaseString));
    } else {
        static char *modeOptionalDef = "DCR";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(modeOptionalDef), strlen(modeOptionalDef) + 1, 1, kNnopbaseString));
    }
    if (dataFormatOptional) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(dataFormatOptional), strlen(dataFormatOptional) + 1, 2, kNnopbaseString));
    } else {
        static char *dataFormatOptionalDef = "NHWC";
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(dataFormatOptionalDef), strlen(dataFormatOptionalDef) + 1, 2, kNnopbaseString));
    }
